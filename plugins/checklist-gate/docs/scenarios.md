...
## シナリオ D: ToDoリスト的な使い方（1ゲートに複数 ack）
...
## シナリオ E: 天の声（tenno_koe）による応答ブロックと ack

**状況**: Claude がファイルを編集した。そのターン終了時に天の声が違反を検出し、応答をブロックした。

### フロー

1. Claude が `Edit` でファイルを変更する
2. ターン終了時に Stop フックが発火 → Haiku が MEMORY.md に照らして評価
3. 違反を検出 → 応答がブロックされる:
   ```
   [天の声] 計画を列挙せずにファイルを変更した

   gate-ack tenno_koe_cleared --reason "<対応内容>" を実行してください。
   ```
4. Claude が deny メッセージを確認し、指摘に対応する
5. Claude が ack を宣言する:
   ```
   gate-ack tenno_koe_cleared --reason "指摘を確認。変更前に計画を列挙してから実施するよう修正した"
   ```
6. 次のターン終了時に Stop フックが ack を確認 → deny と ack を消費してパス

### ポイント

- deny-first が適用されるため、ブロックを受けた後にのみ `gate-ack tenno_koe_cleared` が受け付けられる
- ack を宣言するまで応答を終了できない（次のターンも含めて保留される）
- `gate-toggle off tenno_koe` でセッション中の天の声評価を一時無効化できる（セッション終了時に自動復帰）

---

## シナリオ F: 天の声による Agent 起動前チェック（deny）

**状況**: オーケストレーター Claude が Agent ツールでサブエージェントを起動しようとした。起動パラメータに手続き的違反を天の声が検出し、deny した。

### フロー

1. Claude が Agent ツールでサブエージェントを起動しようとする
2. PreToolUse フックが発火 → Haiku が MEMORY.md に照らして評価
3. 違反を検出（例: `run_in_background: true` が設定されていない）→ 起動を deny:
   ```
   [天の声] run_in_background が true に設定されていない
   ```
4. Claude がパラメータを修正して再度起動を試みる → 今度は通過

### ポイント

- ack は不要。パラメータを修正して再試行するだけで通過できる
- MEMORY.md のルールに基づいた手続き的違反のみを対象とする
- 成果物の品質（サブエージェントが正しい実装をするかどうか）は評価しない

---

## シナリオ G: 天の声による Agent 完了後の検証（additionalContext）

**状況**: サブエージェントが完了した。PostToolUse フックが発火し、天の声が手続き的ルールへの準拠を確認してオーケストレーターに結果を通知した。

### フロー

1. サブエージェントが完了する
2. PostToolUse フックが発火 → Haiku が MEMORY.md に照らして手続き的ルール準拠を評価
3. 評価結果が additionalContext としてオーケストレーターの Claude に注入される:
   - 違反あり: `[天の声] <手続き的違反内容>`
   - 違反なし: `OK`
4. オーケストレーターはこの通知を踏まえて次のアクションを判断する

### ポイント

- PostToolUse はブロックしない（additionalContext の注入のみ）
- 評価対象は「サブエージェントが MEMORY.md のルール（HOW）に従ったか」のみ
- 成果物の品質・正確性（WHAT）は評価しない（それはサブエージェントの役割）
