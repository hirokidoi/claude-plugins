...
[再度ツール使用 -> PreToolUse -> gate_check.py -> 今度は通過]
  |
  v
[Claude がツールを使用し、Agent ツールを呼ぶ]
  |
  v
PreToolUse hook（matcher: Agent）
  |
  v
agent_pre_checker.py  --->  MEMORY.md  --- 手続き的違反チェック
  |
  +--> 違反あり: deny レスポンス
  +--> 違反なし: 通過
...
PostToolUse hook（matcher: Agent）
  |
  v
agent_watcher.py  --->  MEMORY.md  --- 手続き的ルール準拠チェック
  |
  +--> additionalContext として評価結果をオーケストレーターに注入
...
Stop hook
  |
  v
stop_judge.py  --->  State (SQLite)  --- tenno_koe ack チェック
               --->  MEMORY.md      --- LLM 評価（watch_tools 使用時のみ）
  |
  +--> 違反あり: gate_denies に deny を記録 → block レスポンス（gate-ack tenno_koe_cleared 待ち）
  +--> ack 済み: deny と ack を消費 → 通過
  +--> 条件なし: 通過
```
...
7. **PreToolUse (Agent)**: Agent ツール起動前に agent_pre_checker.py が手続き的違反を検査する
8. **PostToolUse (Agent)**: Agent 完了後に agent_watcher.py が手続き的ルール準拠を検査し、結果を additionalContext として注入する
9. **Stop**: 各応答終了時に stop_judge.py が tenno_koe 評価を実行する（watch_tools 使用時のみ Haiku を呼び出す）
...
- **scripts/gate_check.py**: PreToolUse 時のゲート判定、deny/allow の決定、deny-first の enforce、user-prompt-match の発話検証・キーワードチェック、gate-ack / gate-toggle コマンドの内部処理。`_load_policy()` 内で tenno_koe セクションが有効な場合に `tenno_koe_cleared` ack と `tenno_koe` gate を動的に注入する
- **scripts/stop_judge.py**: Stop フック時の tenno_koe 評価。watch_tools フィルタ通過後に MEMORY.md を読み込み Haiku で評価。違反時は gate_denies に記録してブロック。ack 済みの場合は消費して通過
- **scripts/agent_pre_checker.py**: PreToolUse（Agent）時の手続き的違反チェック。違反あれば deny でサブエージェント起動をブロック
- **scripts/agent_watcher.py**: PostToolUse（Agent）時の手続き的ルール準拠チェック。成果物の品質ではなく HOW（手続き）のみを評価。結果を additionalContext として注入
...
- **policy.json**: ゲートルール・ack 項目の宣言的定義。`tenno_koe` セクションでビルトイン会話モニタリングを設定
...
| ファイル | 責務 |
|---|---|
| hooks/hooks.json | フックイベント（SessionStart / UserPromptSubmit / PreToolUse / PostToolUse / Stop）と実行するスクリプトの対応定義。`$CLAUDE_PLUGIN_ROOT` 環境変数を使って scripts/ 配下の Python スクリプトを直接呼び出す |
...
| scripts/gate_check.py | PreToolUse 時のゲート判定、deny/allow の決定、deny-first の enforce、user-prompt-match の発話検証、gate-ack / gate-toggle コマンドの内部処理（ack 記録・トグル操作）。`_load_policy()` で tenno_koe ビルトイン項目を動的注入 |
| scripts/stop_judge.py | Stop フック時の tenno_koe 会話モニタリング。watch_tools フィルタ → MEMORY.md 読み込み → Haiku 評価 → 違反時 block / ack 済み時通過 |
| scripts/agent_pre_checker.py | PreToolUse（Agent matcher）。Agent 起動前に手続き的違反を Haiku で評価し、違反があれば deny |
| scripts/agent_watcher.py | PostToolUse（Agent matcher）。Agent 完了後に手続き的ルール準拠を Haiku で評価し、結果を additionalContext として注入 |
...
- scripts/gate_check.py, scripts/session_init.py, scripts/stop_judge.py, scripts/agent_pre_checker.py, scripts/agent_watcher.py は `$CLAUDE_PLUGIN_DATA/policy.json` を読み込む
...
- scripts/stop_judge.py, scripts/agent_pre_checker.py, scripts/agent_watcher.py は `~/.claude_config/projects/{project}/memory/MEMORY.md` を読み込む
- scripts/stop_judge.py, scripts/agent_pre_checker.py, scripts/agent_watcher.py は `claude -p --model haiku --no-session-persistence` で LLM を呼び出す
...
- trigger の patterns / except_patterns に対応するマッチ判定ロジックが gate_check.py に必要
...
patterns / except_patterns に `Tool(パターン)` 形式で対象を指定する（Claude Code の permissions と同じ記法）。
